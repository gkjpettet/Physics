...
Protected Class Assert
	...
	#tag Method, Flags = &h0
		Sub AreEqual(expected As VMaths.Vector2, actual As VMaths.Vector2, decimalPoints As Integer = 5, message As String = "")
		  If decimalPoints = 0 Then
		    ...
		    End If
		  Else
		    If expected.Compare(actual, decimalPoints) Then
		      Pass()
		    Else
		      Fail(FailEqualMessage(expected.ToString, actual.ToString), message)
		    End If
	...
	#tag Method, Flags = &h0
		Sub IsNaN(value As Double, message As String = "")
		  If value.IsNotANumber Then
		    Pass
		  Else
		    Fail("[NaN] expected, but was [" + value.ToString + "].", message)
		  End If
